...
def get_zonal_stats(
    cog_url: str,
    polygon: Polygon,
    statistic_methods: list = ["count", "min", "mean", "max", "median"],
    bands: list = [1],
):
    ...
    if not isinstance(statistic_methods, list):
        raise Exception("Provided statistic methods are not structured as list")
    ...
    results = []
    for band in bands:
        band_result = zonal_stats(polygon, cog_url, stats=stats, band=band)
