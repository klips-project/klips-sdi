...
def test_timestamp_from_file_name():
    extracted_timestamp = timestamp_from_file_name("dresden_20191127T1000Z.tif")
...
def test_timestamp_within_range():
    ...
    assert timestamp_within_range(
        datetime(2000, 1, 1), datetime(1999, 1, 1), datetime(2001, 1, 1)
    )
    ...
    assert (
        timestamp_within_range(
            datetime(2000, 1, 1), datetime(1998, 1, 1), datetime(1999, 1, 1)
        )
        == False
    )
    ...
    with pytest.raises(Exception):
        assert timestamp_within_range(
            datetime(2000, 1, 1), datetime(1999, 1, 1), datetime(1998, 1, 1)
        )

    # without end timestamp
    assert timestamp_within_range(datetime(2000, 1, 1), datetime(1999, 1, 1), None)

    # without start timestamp
    assert timestamp_within_range(datetime(2000, 1, 1), None, datetime(2001, 1, 1))

    # without range
    assert timestamp_within_range(datetime(2000, 1, 1), None, None)
